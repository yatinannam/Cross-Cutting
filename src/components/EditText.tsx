...
function EditText(props: EditTextProps) {
  ...
  return (
    <div className="flex flex-col my-3 w-full">
      <label
        className={`mb-1.5 ml-1 text-[13px] font-semibold tracking-wide transition-colors ${isFocused ? "text-primary" : "text-slate-600"}`}
      >
        {props.name}
      </label>
      <div
      ...
      >
        {props.icon === IconType.mail && (
          <MailIcon
            className={`w-[18px] h-[18px] transition-colors ${isFocused ? "text-primary" : "text-slate-400"}`}
          />
        )}
