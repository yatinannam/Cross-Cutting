      ...
      <div className="flex flex-col gap-2.5 sm:grid sm:grid-cols-2 lg:gap-3">
        {resolvedOptions.map((opt) => (
          ...
          >
            ...
            <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-[1.5px] transition-colors ${value === opt.score ? "border-primary" : "border-slate-300 group-hover:border-slate-400"}`}>
              {value === opt.score && <div className="h-2.5 w-2.5 rounded-full bg-primary animate-in zoom-in-50 duration-200" />}
            </div>
            {value === opt.score && (
